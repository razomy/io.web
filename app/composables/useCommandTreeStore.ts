...
import {getCommandBy, isCommandExists, getCommandById, type IoConfig} from '~~/razomy/_db/commands';
import {type IoDirectory} from '~~/razomy/io/command/directory';
import {
  getDirectoryByFilter,
  isDirectoryExists,
  getDirectoryBy,
  flatDirectories
} from '~~/razomy/_db/directories';
import {systemSet} from '~~/razomy/_db/systems';
import type {IoCommand, IoCommandKey} from "~~/razomy/io/command";
import {
  defaultIoConfig,
  loadNpmPackageCommands,
  loadNpmPackageCommandsWithCommands
} from "~~/razomy/_db/loadNpmPackageCommands";
...
export const useCommandTreeStore = defineStore('commandTreeStore', {
  state: () => ({
    ...
    commands: [] as IoCommand[],
    directories: [] as IoDirectory[],
    directoriesTree: [] as IoDirectory[],
    systemSet,
  }),

  actions: {
    getCommandBy(...args: DropFirst<Parameters<typeof getCommandBy>>) {
      return getCommandBy(this.commands, ...args)
    },
    isCommandExists(...args: DropFirst<Parameters<typeof isCommandExists>>) {
      return isCommandExists(this.commands, ...args)
    },
    getCommandById(...args: DropFirst<Parameters<typeof getCommandById>>) {
      return getCommandById(this.commands, ...args)
    },
    getDirectoryByFilter(...args: DropFirst<Parameters<typeof getDirectoryByFilter>>) {
      return getDirectoryByFilter(this.directories, ...args)
    },
    isDirectoryExists(...args: DropFirst<Parameters<typeof isDirectoryExists>>) {
      return isDirectoryExists(this.directories, ...args)
    },
    getDirectoryBy(...args: DropFirst<Parameters<typeof getDirectoryBy>>) {
      return getDirectoryBy(this.directories, ...args)
    },
    saveConfigToStorage() {
