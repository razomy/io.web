<template>
    <div class="bg-surface-light min-vh-100">
      <!-- Hero Header -->
      <div class="bg-primary pt-8 pb-24 px-4 text-center">
        <v-container max-width="900">
          <Breadcrumbs />

          <v-avatar color="white" size="80" variant="flat" class="mb-6 mt-4 elevation-3">
            <v-icon :icon="icon" color="primary" size="40" />
          </v-avatar>

          <h1 class="text-h3 font-weight-black text-white mb-2">
            {{ t('fs.source.title', { source: source.toUpperCase() }) }}
          </h1>
          <p class="text-white-70 text-h6 font-weight-regular">
            {{ t('fs.source.subtitle', { source: source.toUpperCase() }) }}
          </p>
        </v-container>
      </div>

      <!-- Список доступных конвертаций -->
      <v-container class="mt-n16 position-relative z-index-1" max-width="1000">
        <v-card class="rounded-xl pa-6" elevation="4">
          <h2 class="text-h5 font-weight-bold mb-6">
            {{ t('fs.source.available_conversions') }}
          </h2>

          <v-row>
            <v-col
                v-for="target in targets"
                :key="target"
                cols="12" sm="6" md="4"
            >
              <v-card
                  :to="localePath(`/${source}/${target}`)"
                  variant="outlined"
                  class="d-flex align-center pa-4 hover-card"
                  color="primary"
              >
                <v-icon icon="mdi-file-swap-outline" start />
                <span class="font-weight-bold text-body-1 text-high-emphasis">
                  {{ source.toUpperCase() }}
                  <span class="text-medium-emphasis mx-1">to</span>
                  {{ target.toUpperCase() }}
                </span>
                <v-spacer />
                <v-icon icon="mdi-chevron-right" color="grey" />
              </v-card>
            </v-col>
          </v-row>
        </v-card>
      </v-container>
    </div>
</template>

<script setup lang="ts">
import Breadcrumbs from '~~/razomy/vue.nuxt/Breadcrumbs.vue';
import { ALLOWED_CONVERSIONS, getFileIcon } from '~~/content/context';
const { t } = useI18n();
const localePath = useLocalePath();

const route = useRoute();
const source = (route.params.source as string).toLowerCase().replace('fs','');

// Проверяем, существует ли такой формат
if (!ALLOWED_CONVERSIONS[source as keyof typeof ALLOWED_CONVERSIONS]) {
  throw createError({ statusCode: 404, statusMessage: 'Format not found' + source });
}

const targets = ALLOWED_CONVERSIONS[source as keyof typeof ALLOWED_CONVERSIONS];
const icon = getFileIcon(source);
</script>

<style scoped>
.hover-card { transition: background 0.2s; border-color: rgba(var(--v-theme-primary), 0.2); }
.hover-card:hover { background: rgba(var(--v-theme-primary), 0.05); border-color: rgb(var(--v-theme-primary)); }
.text-white-70 { color: rgba(255,255,255, 0.7); }
</style>
