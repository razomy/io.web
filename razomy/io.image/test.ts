...
async function runTests() {
  ...
  for (const imgConfig of images) {
    ...
    const inputPath = path.join(OUT_DIR, inputFilename);
    ...
    for (const targetFormat of imgConfig.conversions) {
      const tempInput = path.join(OUT_DIR, `temp_${Date.now()}_${inputFilename}`);
      ...
      try {
        ...
        const result = await processImageStream(tempInput, targetFormat);
  ...
  // fs.rmSync(OUT_DIR, { recursive: true, force: true });
  console.log(`\n🏁 Тесты завершены. Результаты в папке ${OUT_DIR}`);
}
